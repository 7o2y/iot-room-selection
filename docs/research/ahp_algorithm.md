...
```
   ...
   │         │          │         │           │         │
┌──▼───┐ ┌───▼────┐  ┌──▼──┐ ┌───▼───┐   ┌───▼────┐ ┌───▼───┐
│ Temp │ │Lighting│  │ CO2 │ │  VOC  │   │ Seats  │ │A/V    │
│      │ │        │  │     │ │       │   │        │ │       │
│Noise │ │Humidity│  │ AQI │           │Equipment│
└──────┘ └────────┘  └─────┘           └────────┘
```
...
| Sub-Criterion | Parent | Sensor Source |
|---------------|--------|---------------|
| Temperature | Comfort | `temperature_sensor_data.json` |
| Lighting | Comfort | `LightIntensity_sensor_data.json` |
| Noise | Comfort | `sound_sensor_data.json` |
| Humidity | Comfort | `humidity_sensor_data.json` |
| CO2 | Health | `co2_sensor_data.json` |
| Air Quality | Health | `air_quality_sensor_data.json` |
| VOC | Health | `voc_sensor_data.json` |
...
### All Thresholds (Category II EN 16798-1)

| Criterion | Optimal Range | Acceptable Range |
|-----------|---------------|------------------|
| Temperature | 20-24°C | 18-26°C |
| CO2 | < 600 ppm | < 1000 ppm |
| Humidity | 40-60% | 30-70% |
| Lighting | 300-500 lux | 200-750 lux |
| Noise | < 35 dBA | < 45 dBA |
| VOC | < 200 ppb | < 400 ppb |
| AQI | < 50 | < 100 |
...
2. **Accept** user preference comparisons (optional)
3. **Calculate** weights via eigenvector method
4. **Validate** consistency (CR < 0.1)
5. **Map** each sensor reading to 0-1 score
6. **Aggregate** bottom-up through hierarchy
7. **Rank** rooms by final score
...
# Set requirements
engine.set_requirements(UserRequirements(required_seats=20))
