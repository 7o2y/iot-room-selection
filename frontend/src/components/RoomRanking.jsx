...
function RoomRanking({ rankings = [], consistencyRatio = 0 }) {
  ...
  const getStatusBadge = (value, optimalMin, optimalMax) => {
    const isOptimal = value >= optimalMin && value <= optimalMax
    const isAcceptable = value >= optimalMin * 0.9 && value <= optimalMax * 1.1
          ...
          <table className="w-full">
            ...
            <tbody>
              {rankings.map((room) => {
                ...
                const tempStatus = getStatusBadge(room.temperature, 20, 24)
                const co2Status = getStatusBadge(room.co2, 0, 600)
