...
function PreferenceMatrix({ onPreferencesChange }) {
  // Main criteria for room selection
  const criteria = ['Comfort', 'Health', 'Usability']
  ...
  // Calculated weights and consistency ratio
  const [results, setResults] = useState({
    weights: { Comfort: 0.45, Health: 0.35, Usability: 0.20 },
    consistencyRatio: 0.05,
    isConsistent: true,
  })

  // Recalculate weights whenever comparisons change
  useEffect(() => {
    const newResults = calculateAHPWeights(criteria, comparisons)
    ...
    if (onPreferencesChange) {
      onPreferencesChange({
        comparisons,
        weights: newResults.weights,
        consistencyRatio: newResults.consistencyRatio,
      })
    }
  }, [comparisons])
